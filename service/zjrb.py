...
logger = Logger(logname= 'newspaper',logger='zjrb').getlog()
    ...
    try:
        while flag:
            ...
            if resp.status_code == 200:
                ...
                ul = soup.find_all('ul',attrs={'class':'main-ed-articlenav-list'})
                for a in ul:
                    for link in a.find_all('a'):
                        path = link.get('href')
                        title = link.get_text().strip().replace('\r','').replace('\n','')
                        ...
                        if len(title.strip()) <= 5:
                            ...
                            if resp1.status_code == 200:
                                ...
                                list.append('{}###{}###浙江日报###{}###{}'.format(realpath,title,nohtml_content,content))
                            time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================浙江日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
