...
logger = Logger(logname= 'newspaper',logger='lnrb').getlog()
                ...
                for link in div.find_all('a'):
                    ...
                    if len(title.strip()) <= 5:
                        ...
                        if resp1.status_code == 200:
                            ...
                            list.append('{}###{}###辽宁日报###{}###{}'.format(realpath,title,nohtml_content,content))
                        time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================辽宁日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
