...
logger = Logger(logname= 'newspaper',logger='scrb').getlog()
    ...
    try:
        while flag:
            ...
            if resp.status_code == 200:
                ...
                ul = soup.find('div',attrs={'class':'main_r'}).find_all('ul')[2]
                for link in ul.find_all('a'):
                    ...
                    if len(title.strip()) <= 8:
                        ...
                        if resp1.status_code == 200:
                            ...
                            nohtml_content = bsoup.find('font').get_text().strip()
                            content = bsoup.find('font')
                            list.append('{}###{}###四川日报###{}###{}'.format(realpath,title,nohtml_content,content))
                        time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================四川日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
