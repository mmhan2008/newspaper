...
import random
logger = Logger(logname= 'newspaper',logger='hebrb').getlog()
    ...
    try:
        while flag:
            ...
            if i < 11:
                ...
                td = soup.find_all('td',attrs={'class':'black'})
                for str in td:
                    for link in str.find_all('a'):
                        path = link.get('href')
                        title = link.get_text()
                        realpath = parse.urljoin(url,path)
                        if '版' in title or len(title.strip()) <= 4 :
                            pass
                        else:
                            ...
                            if resp1.status_code == 200:
                                ...
                                list.append('{}###{}###哈尔滨日报###{}###{}'.format(realpath,title,nohtml_content,content))
                            time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================哈尔滨日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
