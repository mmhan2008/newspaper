...
import random
logger = Logger(logname= 'newspaper',logger='jlrb').getlog()
                ...
                for link in ul.find_all('a'):
                    ...
                    if len(title.strip()) <= 4:
                        ...
                        if resp1.status_code == 200:
                            ...
                            list.append('{}###{}###吉林日报###{}###{}'.format(realpath,title,nohtml_content,content))
                        time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================吉林日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
