                ...
                for link in div.find_all('a'):
                    ...
                    if len(title.strip()) < 5:
                        ...
                        if resp1.status_code == 200:
                            ...
                            nohtml_content = bsoup.find('span', attrs={'id': 'contenttext'}).get_text().strip()
                            content = bsoup.find('span', attrs={'id': 'contenttext'})
                            list.append('{}###{}###贵州日报###{}###{}'.format(realpath,title,nohtml_content,content))
                        time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================贵州日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
