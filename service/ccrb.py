...
logger = Logger(logname= 'newspaper',logger='ccrb').getlog()
    ...
    try:
        while flag:
            ...
            if resp.status_code == 200:
                ...
                ul = soup.find('ul',attrs={'id':'articlelist'})
                for link in ul.find_all('a'):
                    path = link.get('href')
                    title = link.get_text()
                    realpath = parse.urljoin(url,path)
                    if title.strip() == '' or '广告' in title:
                        pass
                    else:
                        ...
                        if resp1.status_code == 200:
                            ...
                            list.append('{}###{}###长春日报###{}###{}'.format(realpath,title,nohtml_content,content))
                        time.sleep(random.randint(0,2))
            else:
                flag = False
    except Exception as e:
        logger.info('=========================长春日报解析异常=========================')
        logger.info(url)
        logger.info(e)
    finally:
        return list
